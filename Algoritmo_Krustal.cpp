...
/***********************************************************
   ...
   - mst[] guardará únicamente las aristas elegidas por Kruskal.
...
int N, E;   // N vértices, E aristas ingresadas por el usuario
...
void hacerSet(int n)
{
    ...
    for(i = 0; i < n; i++)
    {
        padre[i] = i;      // Un nodo es padre de sí mismo
...
/***********************************************************
   ...
   También aplica compresión de caminos para acelerar.
***********************************************************/
int encontrarPadre(int x)
{
    ...
    // Compresión de caminos: apuntamos directo al representante final
...
{
    // Ordenar las aristas según su peso (menor primero)
    sort(aristas, aristas + E, compararAristas);

    // Se prepara Union-Find
    hacerSet(N);

    int contador = 0;   // Contará cuántas aristas lleva el MST
    int i;
...
/***********************************************************
   ...
   - Pide cantidad de vértices y aristas.
...
void capturarGrafo()
{
    ...
    cout << "\nSe cargó correctamente un grafo NO dirigido.\n";
}
