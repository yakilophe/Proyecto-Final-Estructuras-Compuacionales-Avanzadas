...
bool eliminar[300];    // marca si la arista será borrada
...
void dfs(int u, bool matriz[100][100])
{
    ...
    for(v = 0; v < N; v++)
    {
        // Si existe conexión y no fue visitado, avanzar
...
{
    // Ordenamos aristas de mayor ? menor
    sort(aristas, aristas + E, ordenarDesc);

    // Inicialmente ninguna está eliminada
    int i;
    for(i = 0; i < E; i++)
        eliminar[i] = false;
    ...
    /*********************************************************
       ...
       Aquí añadimos TODAS las aristas primero.
    *********************************************************/
    bool matriz[100][100];
    int u, v;
    ...
    *********************************************************/
    for(i = 0; i < E; i++)
    {
        ...
        // Si se desconectó, la arista NO puede eliminarse
        if(!conectado)
        {
            matriz[a][b] = true;
            matriz[b][a] = true;
